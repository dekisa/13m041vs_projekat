...
if os.name == "nt":
    ...
    NIOS2_TERMINAL_PATH=r"C:\intelFPGA_lite\17.1\quartus\bin64\nios2-terminal.exe"
else:
    START_STRING="PROGRAM START\n"
    NIOS2_TERMINAL_PATH=r"/home/strahinja/tools/intelFPGA_lite/17.1/quartus/bin/nios2-terminal"
        ...
        for data in datas:
            if start_found == False:
                ...
                if counter == 0:
                    ...
                    assert(data == 't')
                    d1 = np.fromstring(databuf[:2], dtype='<i2')[0] / 16384.
                    d2 = np.fromstring(databuf[2:4], dtype='<i2')[0] / 16384.
                    result = np.fromstring(databuf[-2:], dtype='<i2')[0] / 16384.
                    print "d1={:.4f} d2={:.4f} res={:.4f} raw={}".format(d1, d2, result, np.array(bytearray(databuf)))
                    databuf = ''
                    counter = 0
                else:
                    if os.name == "nt":
...
if __name__ == '__main__':
    proc = subprocess.Popen([NIOS2_TERMINAL_PATH, '--no-quit-on-ctrl-d', '--instance=0'], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    try:
