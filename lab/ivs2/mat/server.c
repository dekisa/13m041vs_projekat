...
typedef struct FIFO_PTR_STRUCT {
	uint32_t base;
	uint32_t csr_base;
	uint32_t cid;
	uint32_t irq;
...
static const fifo_ptr_t fifos [] =
{
	{
		.base = FIFO_CLIENT0_OUT_BASE,
		.csr_base = FIFO_CLIENT0_OUT_CSR_BASE,
		.cid = FIFO_CLIENT0_OUT_CSR_IRQ_INTERRUPT_CONTROLLER_ID,
		.irq = FIFO_CLIENT0_OUT_CSR_IRQ,
		.res = 100000
	},
	{
		.base = FIFO_CLIENT1_OUT_BASE,
		.csr_base = FIFO_CLIENT1_OUT_CSR_BASE,
		.cid = FIFO_CLIENT1_OUT_CSR_IRQ_INTERRUPT_CONTROLLER_ID,
		.irq = FIFO_CLIENT1_OUT_CSR_IRQ,
		.res = 20000
	},
	{
		.base = FIFO_CLIENT2_OUT_BASE,
		.csr_base = FIFO_CLIENT2_OUT_CSR_BASE,
		.cid = FIFO_CLIENT2_OUT_CSR_IRQ_INTERRUPT_CONTROLLER_ID,
		.irq = FIFO_CLIENT2_OUT_CSR_IRQ,
		.res = 10000
	}
};

static altera_avalon_mailbox_dev * mboxes [3];
...
static void fifo_handle(void *context)
{
	fifo_ptr_t *fifo_inst = (fifo_ptr_t *)context;

	while (ALTERA_AVALON_FIFO_STATUS_E_MSK != altera_avalon_fifo_read_status(fifo_inst->csr_base, ALTERA_AVALON_FIFO_STATUS_E_MSK))
	{
		count.valid += altera_avalon_fifo_read_fifo(fifo_inst->base, fifo_inst->csr_base);
		count.total += fifo_inst->res;
	}

	recalculate = 1;

	altera_avalon_fifo_clear_event(fifo_inst->csr_base, ALTERA_AVALON_FIFO_EVENT_AF_MSK);
}
...
int main()
{
    ...
    for (int i=0; i<1; i++)
    {
		altera_avalon_fifo_init(fifos[i].csr_base, ALTERA_AVALON_FIFO_STATUS_AF_MSK, 1, 1);
		alt_ic_isr_register(fifos[i].cid,				// cid
							fifos[i].irq,				// irq nr
							fifo_handle,				// isr
							(void *)&fifos[i],			// ctxt
							0x0);						// reserved
    ...
    // initialize mailbox
    mboxes[0] = altera_avalon_mailbox_open("/dev/mbox_client0", NULL, NULL);
//    mboxes[1] = altera_avalon_mailbox_open("/dev/mbox_client1", NULL, NULL);
//    mboxes[2] = altera_avalon_mailbox_open("/dev/mbox_client2", NULL, NULL);
    ...
    for (;;)
    {
    	if (mode != mode_next)
    	{
    		...
    		message[0] = mode;
    	    // send instructions to mailbox
    	    for (int i=0; i<1; i++)
    	    {
    	    	message[1] = fifos[i].res;
